...
async def deploy_contract(w3, contract_type, contract_name, private_key):
    ...
    print(
        f"🤑 Current wallet balance: {Colors.YELLOW}{balance_eth:.6f} MON{Colors.END}"
    )
    ...
    try:
        ...
        # Add 10% boosting
        gas_limit = int(estimated_gas * 1.005)  # edit it
        print(
    ...
    if balance < max_gas_cost:
        print(
            f"❌ Insufficient balance for gas! isi bang {Colors.RED}{max_gas_cost_eth:.6f}{Colors.END} MON but have {Colors.YELLOW}{balance_eth:.6f} MON{Colors.END}"
        )
        return None
    ...
    try:
        ...
        if tx_receipt.status == 1:
            ...
            print(
                f"💲  Actual gas cost: {Colors.YELLOW}{actual_gas_cost_eth:.6f} MON{Colors.END}"
            )
...
async def main():
    ...
    for idx, private_key in enumerate(private_keys):
        try:
            ...
            print(
                f"   Wallet {idx+1}: {Colors.CYAN}{wallet_address[:6]}...{wallet_address[-4:]}{Colors.END} | Balance: {Colors.YELLOW}{balance_eth:.6f} MON{Colors.END}"
            )
    ...
    print(f"   Chain ID: {chain_id} {Colors.HEADER}Monad Testnet {Colors.END}")
