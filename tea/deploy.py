...
def connect_to_rpc():
    ...
    raise ConnectionError("Unable to connect to TEA network. Check RPC URLs.")
...
def estimate_gas(w3, contract_func, sender):
    ...
    try:
        ...
        return int(gas_estimate * 1.11)  # 10% buffer
    except Exception as e:
        default_gas = 150000
...
def track_gas_usage(w3, tx_receipt, gas_price):
    ...
    print_info(f"📊 Gas used: {gas_used} | Biaya Cost: {cost_eth:.8f} {CHAIN_SYMBOLS.get(w3.eth.chain_id, 'TEA')}")
...
async def deploy_contract(w3, current_rpc, contract_type, contract_name, private_key, attempt=0):
    ...
    print_info(f"🤑 Current wallet balance: {Fore.YELLOW}{balance_eth:.6f} TEA{Style.RESET_ALL}")
    ...
    print_info(f"💲 Maximum gas cost: {Fore.YELLOW}{max_gas_cost_eth:.6f} TEA{Style.RESET_ALL}")

    if balance < max_gas_cost:
        print_error(f"❌ Insufficient balance for gas! Need {Fore.RED}{max_gas_cost_eth:.6f}{Style.RESET_ALL} TEA but have {Fore.YELLOW}{balance_eth:.6f} TEA{Style.RESET_ALL}")
        return None
    tx_data = {}
    ...
    try:
        ...
        if tx_receipt and tx_receipt.status == 1:
            ...
            print_info(f"💲 Actual gas cost: {Fore.YELLOW}{actual_gas_cost_eth:.6f} TEA{Style.RESET_ALL}")
...
async def main():
    ...
    for idx, private_key in enumerate(private_keys):
        try:
            ...
            print(f"   Wallet {idx+1}: {Fore.CYAN}{short_address(wallet_address)}{Style.RESET_ALL} | Balance: {Fore.YELLOW}{balance_eth:.6f} TEA{Style.RESET_ALL}")

            if balance_eth >= 0.05:
                valid_wallets.append(private_key)
            else:
                print_warning(f"   ⚠️ Low balance (< 0.05 TEA) for wallet {short_address(wallet_address)}, may not be sufficient for gas")
        except Exception as e:
    ...
    print(f"   Chain ID: {chain_id} {Fore.MAGENTA}TEA-Sepolia {Style.RESET_ALL}")
    print(f"   Connected to RPC: {Fore.GREEN}{w3.provider.endpoint_uri}{Style.RESET_ALL}")

    # Get the total number of contracts to deploy 8
    total_contracts_per_wallet = 8
    ...
    for cycle in range(total_contracts_per_wallet):
        ...
        if cycle < total_contracts_per_wallet - 1:
            # Random wait time between 2-4 hours
            wait_hours = random.uniform(2.0, 4.0)
