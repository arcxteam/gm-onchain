...
UNSTAKE_DELAY_MIN = 2000  # 30mins
UNSTAKE_DELAY_MAX = 3000  # 60mins
WALLET_INTERVAL_MIN = 300  # 5mins
WALLET_INTERVAL_MAX = 500  # 10mins
RETRY_DELAY = 50
MAX_RETRIES = 3

# CONFIG STAKING
MIN_STAKE_AMOUNT = 0.11
MAX_STAKE_AMOUNT = 0.12
...
CONTRACT_ADDRESS = "0x04290DACdb061C6C9A0B9735556744be49A64012"

# FUNCTION SIGN
STAKE_SELECTOR = "0x3a4b66f1"
UNSTAKE_SELECTOR = "0x2e1a7d4d"
...
print(f"{Fore.GREEN}======================= WELCOME TO TEA Protocol ONCHAIN ========================{Fore.RESET}")
...
{Fore.CYAN}        Welcome to TEA Protocol Onchain Testnet & Mainnet Interactive {Fore.RESET}
...
async def stake_tea(web3, wallet, wallet_idx):
    """Stake TEA with random amount"""
    try:
        # Get account balance
        balance = get_wallet_balance(web3, wallet.address)
        print(f"💰 Wallet {Fore.YELLOW}[{wallet_idx}]{Fore.RESET} checking balance: {Fore.YELLOW}{balance:.6f} TEA{Style.RESET_ALL}")
        ...
        print(f"🔄 Wallet {Fore.YELLOW}[{wallet_idx}]{Fore.RESET} be ready staking: {Fore.MAGENTA}{amount_float:.6f} TEA{Style.RESET_ALL}")
        ...
        print(f"💰 Update new balance: {Fore.YELLOW}{new_balance:.6f} TEA{Style.RESET_ALL}")
...
async def unstake_tea(web3, wallet, wallet_idx):
    """Unstake TEA tokens"""
    try:
        ...
        print(f"🔄 Wallet {Fore.YELLOW}[{wallet_idx}]{Fore.RESET} unstaking: {Fore.YELLOW}{amount_eth:.6f} TEA{Style.RESET_ALL}")
        ...
        new_balance = get_wallet_balance(web3, wallet.address)
        print(f"💰 Update new balance after unstake: {Fore.YELLOW}{new_balance:.6f} TEA{Style.RESET_ALL}")
...
async def process_wallet(wallet, wallet_idx, cycle):
    ...
    try:
        ...
        # Step 1: Stake TEA
        result = await stake_tea(web3, wallet, wallet_idx)
        if not result:
        ...
        # Step 2: Unstake TEA
        unstake_success = await unstake_tea(web3, wallet, wallet_idx)
...
async def run_wallet_continuously(wallet, wallet_idx):
    ...
    while True:
        try:
            ...
            inter_cycle_delay = random.randint(90, 185)  # 21-61 seconds
...
async def main():
    ...
    try:
        ...
        print(f"🚀  Starting {Fore.MAGENTA}TEA Stake{Fore.RESET} Liquid Staking Unstaking Automation bang...")
        ...
        print(f"\n{Fore.MAGENTA}Thank you for using TEA Stake Staking utomation{Style.RESET_ALL}")
