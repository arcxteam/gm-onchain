...
try:
    ...
    print(
        f"Warning: Could not install solc 0.8.17, will use available version. Error: {e}"
    )
...
async def deploy_contract(w3, contract_type, contract_name, private_key):
    ...
    print(
        f"⚙️ {Colors.HEADER} Compiling {Colors.YELLOW}{contract_type}{Colors.HEADER} the contract name is {Colors.YELLOW}{contract_name}{Colors.END}"
    )
    ...
    print(
        f"🤑 Current wallet balance: {Colors.YELLOW}{balance_eth:.6f} A0GI{Colors.END}"
    )

    try:
        gas_price = w3.eth.gas_price
        increased_gas_price = max(int(gas_price * 1.31), w3.to_wei(13, "gwei"))
        print(
            f"💰 Using gas price: {Colors.YELLOW}{w3.from_wei(increased_gas_price, 'gwei')} gwei{Colors.END}"
        )
    except Exception as e:
        print(f"{Colors.HEADER}⚠️ Could not get gas price: {str(e)}{Colors.END}")
        increased_gas_price = w3.to_wei(13, "gwei")  # edit gwei fallback
        print(f"{Colors.HEADER}💰 Using fallback gas price: 13 gwei{Colors.END}")
    ...
    try:
        ...
        estimated_gas = w3.eth.estimate_gas(
            {"from": wallet_address, "data": contract_data["bytecode"]}
        )
        # Add boosting
        gas_limit = int(estimated_gas * 1.01)  # edit it
        print(
            f"⛽ Estimated gas: {Colors.YELLOW}{estimated_gas}{Colors.END} -> Boosting gas 5%...{Colors.YELLOW} final gas is {gas_limit}{Colors.END}"
        )
    except Exception as e:
    ...
    print(f"💲 Maximum gas cost: {Colors.YELLOW}{max_gas_cost_eth:.6f} A0GI{Colors.END}")

    if balance < max_gas_cost:
        print(
            f"❌ Insufficient balance for gas! isi bang {Colors.RED}{max_gas_cost_eth:.6f}{Colors.END} A0GI but have {Colors.YELLOW}{balance_eth:.6f} A0GI{Colors.END}"
        )
        return None
    ...
    print(
        f"{Colors.HEADER}🚀  Deploying contract to blockchain...WAIT...WAIT{Colors.END}"
    )
    ...
    try:
        ...
        print(
            f"📨 Transaction explorer TXiD: {Colors.CYAN}{w3.to_hex(tx_hash)}{Colors.END}"
        )

        print(f"{Colors.YELLOW}⏳ Waiting for transaction confirmation...{Colors.END}")
        tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=210)
        ...
        if tx_receipt.status == 1:
            ...
            print(
                f"📝 Transaction TXiD/Hash: {Colors.CYAN}{w3.to_hex(tx_hash)}{Colors.END}"
            )
            print(
                f"⛽ Get gas used: {Colors.YELLOW}{tx_receipt.gasUsed}{Colors.END} {Colors.GREEN}{(tx_receipt.gasUsed / gas_limit) * 100:.1f}% no limit bang{Colors.END}"
            )
            ...
            print(
                f"💲  Actual gas cost: {Colors.YELLOW}{actual_gas_cost_eth:.6f} A0GI{Colors.END}"
            )
            ...
            print(
                f"❌ Deployment {Colors.RED}failed.{Colors.END} Transaction status: {Colors.RED}{tx_receipt.status}{Colors.END}"
            )
            try:
                tx_info = w3.eth.get_transaction(tx_hash)
                print(
                    f"📊 Transaction details: Gas price {Colors.YELLOW}{w3.from_wei(tx_info['gasPrice'], 'gwei')} gwei, Gas limit {tx_info['gas']}{Colors.END}"
                )
            except:
                pass
            return None
...
async def wait_with_progress(hours, message="Waiting"):
    ...
    while time.time() < end_time:
        ...
        print(
            f"⌛ Progress: {Colors.YELLOW}{progress_percent:.1f}%{Colors.END} | Elapsed: {Colors.YELLOW}{elapsed_hours:.1f}h{Colors.END} | Remaining: {Colors.YELLOW}{remaining_hours:.1f}h{Colors.END}",
            end="\r",
        )
...
{Colors.HEADER}    Welcome to Footprint Onchain Testnet & Mainnet Interactive {Colors.END}
...
def load_private_keys():
    ...
    print(
        f"📸 Loaded {len(private_keys)} EVM wallet {Colors.GREEN}Successfully{Colors.END}"
    )
...
def get_contract_types_for_deployment(num_contracts=3):
    ...
    print(
        f"🔍 Selected contract types: {Colors.YELLOW}{', '.join(selected_types)}{Colors.END}"
    )
...
async def main():
    ...
    if not private_keys:
        print(
            f"❌ No private keys found. Please check your .env file or private_keys.txt"
        )
        return
    ...
    for rpc_url in RPC_URLS:
        try:
            print(f"🔄 Already to connect RPC {Colors.HEADER}{rpc_url}{Colors.END}")
            ...
            if w3_temp.is_connected():
                w3 = w3_temp
                print(
                    f"✅ Successfully connected to RPC {Colors.GREEN}{rpc_url}{Colors.END}"
                )
                break
        except Exception as e:
            print(
                f"{Colors.RED}❌ Failed to connect to {rpc_url}: {str(e)}{Colors.END}"
            )
    ...
    for idx, private_key in enumerate(private_keys):
        try:
            ...
            print(
                f"   Wallet {idx+1}: {Colors.CYAN}{wallet_address[:6]}...{wallet_address[-4:]}{Colors.END} | Balance: {Colors.YELLOW}{balance_eth:.6f} A0GI{Colors.END}"
            )

            if balance_eth >= 0.05:
                valid_wallets.append(private_key)
            else:
                print(f" ⚠️ Low balance (< 0.05 0G), may not be sufficient for gas")
        except Exception as e:
            print(
                f"{Colors.RED}   ❌ Error checking wallet {idx+1}: {str(e)}{Colors.END}"
            )
    ...
    print(f"   Chain ID: {chain_id} {Colors.HEADER}0G-Newton-Testnet {Colors.END}")
    print(f"   Connected to RPC: {Colors.GREEN}{w3.provider.endpoint_uri}{Colors.END}")
    ...
    print(
        f"🚀{Colors.YELLOW} Will deploy {total_contracts} contracts total ({total_contracts_per_wallet} per wallet){Colors.END}"
    )
    print(
        f"   Strategy: Deploy contracts sequentially across all wallets first, then wait 12-hour between cycles"
    )
    print(
        f"   Estimated completion time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} + 24 hours"
    )
    print(f"   No user interaction will be required during the 24-hour period.")
    print(
        f"   Deploy results will {Colors.RED}not be saved{Colors.END} in favour of keep the {Colors.GREEN}clean{Colors.END} folder + storage in use."
    )

    # Give user 13 seconds to cancel if needed
    print(
        f"{Colors.HEADER}⚠️ Starting in 13 seconds. Press Ctrl+C to cancel...{Colors.END}"
    )
    try:
    ...
    for cycle in range(total_contracts_per_wallet):
        ...
        for wallet_idx, wallet_key in enumerate(valid_wallets):
            ...
            print(
                f"{Colors.BOLD}🔨 Wallet {wallet_idx+1}/{len(valid_wallets)} Deployment {cycle+1}/{total_contracts_per_wallet} at {deployment_time}{Colors.END}"
            )
            print(
                f"{Colors.BOLD}🔨 Using wallet: {Colors.YELLOW}{wallet_address[:6]}...{wallet_address[-4:]}{Colors.END}"
            )
            print(
                f"{Colors.BOLD}🔨 Deploying: {Colors.YELLOW}{contract_name}{Colors.END} ({Colors.CYAN}{contract_type}{Colors.END})"
            )
            print(
            ...
            deployment = await deploy_contract(
                w3, contract_type, contract_name, wallet_key
            )
            ...
            if deployment:
                ...
                # Short wait between wallets within the same cycle (69-135 seconds)
                if wallet_idx < len(valid_wallets) - 1:
                    wait_seconds = random.randint(69, 135)
                    print(
                        f"{Colors.YELLOW}⏳ Short delay wait in {wait_seconds} seconds before, switch next wallet deployment...{Colors.END}"
                    )
        ...
        if cycle < total_contracts_per_wallet - 1:
            ...
            await wait_with_progress(
                wait_hours,
                f"Completed cycle {cycle+1}/{total_contracts_per_wallet}. Waiting for next cycle",
            )
    ...
    print(
        f" ✅ Total contracts deployed: {Colors.YELLOW}{len(deployments)}/{total_contracts}{Colors.END}"
    )
